...
import scommons.websql.io.showcase.domain.dao._
...
class SqliteContextSpec extends AsyncTestSpec {
  ...
  it should "return non-empty results when getById" in {
    //given
    val db = WebSQL.openDatabase(":memory:")
    val dao = new CategoryDao(new ShowcaseDBContext(db))
    ...
    //then
    result.map { res =>
      res shouldBe Some(CategoryEntity(1, "test category 1"))
    }
  }
  
  it should "return non-empty results when list" in {
    //given
    val db = WebSQL.openDatabase(":memory:")
    val dao = new CategoryDao(new ShowcaseDBContext(db))
